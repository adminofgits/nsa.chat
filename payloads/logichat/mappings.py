...
class Injector(object):
  ...
  def send_left(self, shift=False, ctrl=False, win=False):
    self.protocol.send_hid_event(scan_code=0x50, shift=shift, ctrl=ctrl, win=win)
    self.protocol.send_hid_event(scan_code=0x00)

  def send_right(self, shift=False, ctrl=False, win=False):
    self.protocol.send_hid_event(scan_code=0x4f, shift=shift, ctrl=ctrl, win=win)
    self.protocol.send_hid_event(scan_code=0x00)

  def send_winplusl(self, shift=False, ctrl=False, win=True):
